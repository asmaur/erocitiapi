...
class Subscriptions(viewsets.ViewSet):
    ...
    def general(self, request):
        ...
        queryset = Subscription.subs_active.filter(perfil__category="mulheres").filter(perfil__is_working=True).filter(perfil__suspended=False).order_by("-types", "-end_date",)[:25]
